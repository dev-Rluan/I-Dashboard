...
export default function Ports() {
  ...
  const load = () => api.ports().then(setPorts).catch(() => {})
  useEffect(() => { load() }, [])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <input
            type="text" placeholder="포트·서비스·프로세스 검색"
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {visible.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">결과 없음</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <div className="text-gray-500 text-xs">{visible.length}개 표시 / 전체 {ports.length}개 (5초 자동갱신)</div>
    </div>
  )
}
